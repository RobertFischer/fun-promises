...
describe("FunPromise", () => {
    ...
    describe("resolve/return", () => {
        _.forEach({ static: FunPromise, instance: tokenFunPromise }, (impl, implName) => {
                ...
                describe(methodName, () => {
                    describe(implName, () => {
                        ...
                        it("rejects when provided a rejection", async () => {
                            await expect(doResolve(Promise.reject("BOOM!"))).rejects.toBe("BOOM!");
                        });
    ...
    describe("catch", () => {
        ...
        it("returns the error when not passed an argument", async () => {
            ...
            await expect(tokenFunPromise.then(() => Promise.reject(error)).catch()).resolves.toBe(error);
        });
    });
    describe("arrayify", () => {
        it("basically works", async () => {
            const value = [1, 2, 3, 4];
            await expect(FunPromise.resolve(value).arrayify()).resolves.toStrictEqual(value);
        });
        it("returns a clone", async () => {
            const value = [1, 2, 3, 4];
            await expect(FunPromise.resolve(value)
                .arrayify()
                .then((it) => {
                it.pop();
                return it;
        ...
        it("does not resolve values when called without an argument", async () => {
            ...
            try {
                await expect(FunPromise.resolve([1, 2, rejection]).arrayify()).resolves.toBeArrayOfSize(3);
            }
            finally {
        ...
        it("resolves values when called with the sole argument `true`", async () => {
            await expect(FunPromise.resolve([
                Promise.resolve(1),
        ...
        it("rejects values when called with the sole argument `true` and a rejection in the mix", async () => {
            const rejection = Promise.reject("BOOM!");
            await expect(FunPromise.resolve([1, 2, rejection]).arrayify(true)).rejects.toBe("BOOM!");
        });
        it("rejects values in order when called with the arguments `(true, true)`", async () => {
            ...
            try {
                await expect(FunPromise.resolve([1, 2, rejection1, rejection2]).arrayify(true, true)).rejects.toBe("BOOM!");
            }
            finally {
                ...
                describe("basically works", () => {
                    _.times(10, (argCount) => {
                        ...
                        it(`for ${argCount} arguments`, async () => {
                            await expect(runAll(simpleArgs)).resolves.toStrictEqual(simpleArgs);
                        });
                    });
                });
            });
    ...
    describe("try", () => {
        ...
        it("explodes properly", async () => {
            await expect(FunPromise.try(() => Promise.reject("BOOM!"))).rejects.toBe("BOOM!");
        });
    });
    describe("map", () => {
        _.forEach([true, false], (staticVersion) => {
            describe(staticVersion ? "static" : "instance", () => {
                ...
                it("basically works", async () => {
                    await expect(runMap(["Hello", "Goodbye"], (it) => `${it}, World!`)).resolves.toStrictEqual(["Hello, World!", "Goodbye, World!"]);
                });
                it("works with some test values", async () => {
                    ...
                    await expect(runMap(defaultValues, defaultMapper)).resolves.toStrictEqual([2, 4, 8, 16, 32]);
                });
            });
        });
    });
    ...
    describe("finally", () => {
        ...
        it("executes on resolution", async () => {
            ...
            await expect(FunPromise.resolve(false).finally(() => {
        ...
        it("executes on rejection", async () => {
            let finallyExecuted = false;
            await expect(FunPromise.reject("BOOM!").finally(() => {
                finallyExecuted = true;
            })).rejects.toBe("BOOM!");
            expect(finallyExecuted).toBe(true);
        });
    ...
    describe("coalesce", () => {
        ...
        it("basically works", async () => {
            await expect(runCoalesce([() => null, Promise.resolve(() => true)])).resolves.toBe(true);
        });
        it("returns the first resolving value", async () => {
            await expect(runCoalesce([() => FunPromise.delay(1000).resolve(true), () => false])).resolves.toBe(false);
        });
        it("returns the value that passes the test", async () => {
            await expect(runCoalesce([() => 2, () => 4], (it) => it > 2)).resolves.toBe(4);
        });
        it("throws the error if nothing passes the test", async () => {
            await expect(runCoalesce([() => Promise.reject("BOOM!"), () => 4], (it) => it > 4)).rejects.toBe("BOOM!");
        });
        it("punts appropriately after the first resolution", async () => {
            await expect(runCoalesce(_.times(3, (idx) => () => idx))).resolves.toBe(0);
        });
    });
    describe("delay", () => {
        _.forEach([true, false], (staticVersion) => {
    ...
    describe("simplify", () => {
        it("basically works", async () => {
            await expect(new FunPromise(Promise.resolve(Promise.resolve(true))).simplify()).resolves.toBe(true);
        });
    });
    describe("filter", () => {
        _.forEach([true, false], (staticVersion) => {
    ...
    describe("tap", () => {
        it("basically works", async () => {
            let sawTap = false;
            await expect(tokenFunPromise.tap((val) => {
    ...
    describe("tapCatch", () => {
        it("basically works", async () => {
            ...
            await expect(FunPromise.reject("BOOM!").tapCatch((e) => {
                sawTapCatch = true;
                expect(e).toBe("BOOM!");
                return "BANG!";
            })).rejects.toBe("BOOM!");
        ...
        it("nests explosions", async () => {
            ...
            await expect(FunPromise.reject(err1)
                .tapCatch((e) => {
                throw err2;
            })
                .catch((e) => {
    ...
    describe("tapEach", () => {
        it("basically works", async () => {
            ...
            let count = 0;
            await expect(funPromise.tapEach(() => {
                count++;
    ...
    describe("wrapError", () => {
        it("basically works", async () => {
            const reason = "BOOM!";
            await expect(FunPromise.reject(reason).wrapError("BANG!")).rejects.toHaveProperty("message", "BANG!");
            await expect(FunPromise.reject(reason).wrapError("BANG!")).rejects.toHaveProperty("innerError");
        });
    });
    describe("wrapErrors", () => {
        it("basically works", async () => {
            const err1 = new Error("BOOM!");
            const err2 = new Error("BANG!");
            await expect(FunPromise.resolve([Promise.reject(err1), Promise.reject(err2)])
                .wrapErrors("ERRORS!")
                .catch((e) => {
                expect(e).toBeInstanceOf(NestedError);
    ...
    describe("settle", () => {
        it("basically works for rejections", async () => {
            await expect(FunPromise.reject("BOOM!").settle()).resolves.toHaveProperty("reason", "BOOM!");
        });
        it("basically works for fulfillments", async () => {
            await expect(FunPromise.resolve(true).settle()).resolves.toHaveProperty("value", true);
        });
    });
    describe("settleAll", () => {
        _.forEach([true, false], (staticVersion) => {
            describe(staticVersion ? "static" : "instance", () => {
                ...
                it("basically works", async () => {
                    ...
                    await expect(doSettleAll(_.map(values, "in"))).resolves.toEqual(_.map(values, "out"));
                });
