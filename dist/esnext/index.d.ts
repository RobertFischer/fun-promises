 ...
 */
export declare type Element<T> = Unpromise<T> extends Iterable<infer U>
	? Unpromise<U>
	: never;
/**
 * Provides the type of the resolution value after unwrapping all the [[`PromiseLike`]] and [[`Promisable`]] interfaces (if any).
 */
export declare type Unpromise<T> = T extends PromiseLike<infer U>
	? Unpromise<U>
...
export default class FunPromise<T> implements SimplifiedPromise<T> {
	...
	): FunPromise<(Unpromise<T> | Unpromise<T>)[]>;
	/**
	 ...
	 */
	static coalesce<T>(
		them: PromisableIterable<() => Promisable<T>>
	): FunPromise<any>;
	/**
