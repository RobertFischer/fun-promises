...
export default class FunPromise {
     ...
     */
    map(mapper) {
        return this.reduceArray((ary) => Promise.all(_.map(ary, async (it) => await mapper(it))));
    }
    /**
     * Given a [[`PromisableIterable`]], resolves each value provided and passes each value through the
     * `mapper` function, collecting the results.
    ...
    flatMap(mapper) {
        return this.reduceArray(async (ary) => {
            const results = [];
            await Promise.all(_.map(ary, async (it) => {
                await Promise.all(_.map(_.toArray(await mapper(it)), async (itsItem) => results.push(await itsItem)));
            }));
            return results;
        });
    }
    ...
    flatMapSeq(mapper) {
        return this.reduceArray(async (ary) => {
            ...
            while (!_.isEmpty(ary)) {
                ...
                await Promise.all(_.map(_.toArray(roundResult), async (it) => result.push(await it)));
            }
            return result;
        });
    }
