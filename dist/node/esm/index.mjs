...
import _Symbol$toStringTag2 from "@babel/runtime-corejs3/core-js/symbol/to-string-tag";
...
export default class FunPromise {
  ...
  constructor(wrapped) {
    ...
    this[_Symbol$toStringTag2] = wrapped[_Symbol$toStringTag2];
  }
  /**
  ...
  static wrap(promise) {
    if (promise instanceof FunPromise) {
      return promise;
    } else if (promise instanceof _Promise) {
      return new FunPromise(promise);
    } else {
      return new FunPromise(new _Promise(resolve => resolve(promise)));
    }
  }
  /**
  ...
  static resolve(it) {
    if (it instanceof _Promise || it instanceof FunPromise) {
  ...
  static new(executor) {
    return FunPromise.wrap(new _Promise(executor));
  }
  /**
  ...
  processArray(handler) {
    return FunPromise.wrap(this.arrayify().wrapped.then(async inputIterable => {
      const ary = _clone(_castArray(inputIterable));

      const resultIterable = handler(ary);
      return _Promise.all(_toArray(resultIterable));
    }));
  }
  /**
  ...
  all() {
    return this.processArray(_identity);
  }
  /**
  ...
  race(errorHandler) {
    return this.reduceArray(ary => new _Promise(async (resolver, rejector) => {
      const doResolve = _once(resolver);

      _forEach(ary, it => FunPromise.resolve(it).tapFinally(() => {
        _pull(ary, it);
      }).then(doResolve, e => {
        if (!_isNil(errorHandler)) errorHandler(e);
        if (_isEmpty(ary)) rejector(e);
      }));
    }));
  }
  /**
  ...
  coalesce() {
    return this.reduceArray(async ary => {
      ...
      throw (_lastRejection = lastRejection) != null ? _lastRejection : new Error("No values found to coalesce");
    });
  }
  /**
  ...
  static map(them, mapper) {
    ...
    return _mapInstanceProperty(_context = FunPromise.resolve(them)).call(_context, mapper);
  }
  /**
  ...
  map(mapper) {
    return this.reduceArray(ary => _Promise.all(_map(ary, async it => await mapper(it))));
  }
  /**
   * Given a [[`PromisableIterable`]], resolves each value provided and passes each value through the
   * `mapper` function, collecting the results.
  ...
  static mapSeq(them, mapper) {
    ...
    return _mapInstanceProperty(_context2 = FunPromise.resolve(them)).call(_context2, mapper);
  }
  /**
  ...
  static flatMap(them, mapper) {
    ...
    return _flatMapInstanceProperty(_context3 = FunPromise.resolve(them)).call(_context3, mapper);
  }
  /**
  ...
  flatMap(mapper) {
    return this.reduceArray(async ary => {
      const results = [];
      await _Promise.all(_map(ary, async it => {
        await _Promise.all(_map(_toArray(await mapper(it)), async itsItem => results.push(await itsItem)));
      }));
      return results;
    });
  }
  ...
  flatMapSeq(mapper) {
    return this.reduceArray(async ary => {
      ...
      while (!_isEmpty(ary)) {
        ...
        await _Promise.all(_map(_toArray(roundResult), async it => result.push(await it)));
      }
  ...
  fold(initialValue, handler) {
    return this.reduceArray(ary => {
      ...
      return _Promise.all(_map(ary, async it => {
  ...
  static filter(them, filter) {
    ...
    return _filterInstanceProperty(_context4 = FunPromise.resolve(them)).call(_context4, filter);
  }
  /**
  ...
  filter(test) {
    return this.processArray(async ary => {
      const testResults = await _Promise.all(_map(ary, test));
      return _filter(ary, (item, idx) => {
  ...
  catchError(handler = _identity) {
    return this.catch(reason => {
      if (_isNil(reason)) {
        ...
        handler(new Error(`Promise was rejected; expected Error, but saw: ${_toString(reason)} (${typeof reason})`));
      }
    });
  }
  /**
  ...
  tapFinally(handler, errHandler) {
    return this.tap(handler).tapCatch(errHandler != null ? errHandler : handler);
  }
  /**
  ...
  arrayify() {
    return FunPromise.wrap(this.wrapped.then(iterable => _toArray(iterable)));
  }
  /**
