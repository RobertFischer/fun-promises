...
export default class FunPromise {
	...
	race(errorHandler) {
		return this.reduceArray(
			(ary) =>
				new _Promise(async (resolver, rejector) => {
	...
	static map(them, mapper) {
		...
		return _mapInstanceProperty((_context = FunPromise.resolve(them))).call(
			_context,
			mapper
		);
	}
	...
	map(mapper) {
		return this.reduceArray((ary) =>
			_Promise.all(_map(ary, async (it) => await mapper(it)))
		);
	}
	/**
	 * Given a [[`PromisableIterable`]], resolves each value provided and passes each value through the
	...
	static mapSeq(them, mapper) {
		...
		return _mapInstanceProperty((_context2 = FunPromise.resolve(them))).call(
			_context2,
			mapper
		);
	}
	...
	static flatMap(them, mapper) {
		...
		return _flatMapInstanceProperty(
			(_context3 = FunPromise.resolve(them))
		).call(_context3, mapper);
	}
	/**
	...
	flatMap(mapper) {
		return this.reduceArray(async (ary) => {
			...
			await _Promise.all(
				_map(ary, async (it) => {
					await _Promise.all(
						_map(_toArray(await mapper(it)), async (itsItem) =>
	...
	flatMapSeq(mapper) {
		return this.reduceArray(async (ary) => {
			...
			while (!_isEmpty(ary)) {
				...
				await _Promise.all(
					_map(_toArray(roundResult), async (it) => result.push(await it))
				);
			}
	...
	static filter(them, filter) {
		...
		return _filterInstanceProperty((_context4 = FunPromise.resolve(them))).call(
			_context4,
			filter
		);
	}
	...
	catchError(handler = _identity) {
		return this.catch((reason) => {
			if (_isNil(reason)) {
				...
				handler(
					new Error(
						`Promise was rejected; expected Error, but saw: ${_toString(
							reason
						)} (${typeof reason})`
					)
				);
